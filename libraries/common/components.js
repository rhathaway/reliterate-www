...
prx.commonproperties.backgroundColor = { caption: 'Background', name: 'backgroundColor', type: 'colorpicker', value: function(item,name) { return item.backgroundColor; }, liveUpdate: 'background-color' };
...
prx.commonproperties.borderWidth = { caption: 'Border', name: 'borderWidth', type: 'combo-select', value: function(item,name) { return item.borderWidth; }, values: { min: 0, max: 20, step: 1 } };
prx.commonproperties.borderColor = { caption: false, name: 'borderColor', type: 'colorpicker', value: function(item,name) { return item.borderColor; }, liveUpdate: 'border-color' };
prx.commonproperties.borderRadius = { caption: '<span class="property-icon property-border-radius" title="Border radius"></span>', name: 'borderRadius', type: 'combo-select', value: function(item,name) { return item.borderRadius; }, values: { min: 0, max: 20, step: 1 } };
prx.commonproperties.text = { caption: 'Text', name: 'text', type: 'input', value: function(item,name) { return item.text; } };
prx.commonproperties.textFont = { caption: false, name: 'textFont', type: 'select', value: function(item,name) { return item.textFont; }, values: function(){ return prx.comps.fonts } };
prx.commonproperties.textSize = { caption: false, name: 'textSize', type: 'combo-select', value: function(item,name) { return item.textSize; }, values: prx.comps.textsize };
prx.commonproperties.textColor = { caption: false, name: 'textColor', type: 'colorpicker', value: function(item,name) { return item.textColor; }, liveUpdate: 'color' };
prx.commonproperties.textProperties = { caption: false, name: 'textProperties', type: 'checkbox', value: function(item,name) { if(typeof(item.textProperties) == "undefined") {item.textProperties = [];} return item.textProperties; }, values: [{ value: 'bold', displayValue: '<span class="property-icon property-text-bold" title="Bold"></span>'}, { value: 'italic', displayValue: '<span class="property-icon property-text-italic" title="Italic"></span>'}, { value: 'underline', displayValue: '<span class="property-icon property-text-underline" title="Underine"></span>'}] };
prx.commonproperties.textAlign = { caption: false, name: 'textAlign', type: 'radio', value: function(item,name) { return item.textAlign; }, values: [{ value: 'left', displayValue: '<span class="property-icon property-align-left" title="Align left"></span>'}, { value: 'center', displayValue: '<span class="property-icon property-align-center" title="Align center"></span>'}, { value: 'right', displayValue: '<span class="property-icon property-align-right" title="Align right"></span>'}] };
...
prx.types.text = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cReturn = '<div id="' + _id + '" class="box pos type-text liveUpdate-textColor liveUpdate-backgroundColor" style="line-height: 1.231; color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; background-color: '+getColor(item.backgroundColor)+'; '+_bold+_italic+_underline+_shadow+' text-align: '+item.textAlign+';"><div style="overflow: hidden; width: 100%; height: 100%;"> <span data-editableproperty="text" '+((item.autoResize) ? 'class="autoresize"' : '')+'>'+item.text+'</span></div></div>';
		return cReturn;
	}
	,editableProperties: [
...
prx.types.richtext = {
	name: 'richtext'
		...
		var _id = (!containerid) ? item.id : containerid+'-'+item.id;
		var cR = '';
		cR += '<div id="' + _id + '" class="box pos type-richtext"><div style="overflow: hidden; width: 100%; height: 100%;"><span data-editableproperty="text">' + item.text + '</span></div></div>';
		return cR;
	}
...
prx.types.rectangle = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cReturn = '<div id="' + _id + '" class="box pos type-rectangle"><div id="rec-' + _id + '" class="inner-rec liveUpdate-backgroundColor liveUpdate-borderColor" style="background-color: '+getColor(item.backgroundColor)+'; border: '+item.borderWidth+'px '+item.borderStyle+' ' + getColor(item.borderColor) + '; -moz-border-radius: '+item.borderRadius+'; -webkit-border-radius: '+item.borderRadius+'; border-radius: '+item.borderRadius+';"></div></div>';
		return cReturn;
	}
	,interactions: [
		prx.commonproperties.actions
...
prx.types.circle.propertyGroups = removeProperties(prx.types.circle.propertyGroups, ['borderRadius'])
...
prx.types.horizontalline = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cReturn = '<div id="' + _id + '" class="box pos type-horizontalline" style="height: '+item.weight+'px;"><div class="inner liveUpdate-color" style="border-top: '+item.weight+'px '+item.borderStyle+' '+getColor(item.color)+';"></div></div>';
		return cReturn;
	}
	,propertyGroups: [
		{
...
prx.types.verticalline = {
	...
	,onDisplay: function(item,containerid) {
		...
		if(typeof(item.borderStyle) == "undefined") { item.borderStyle = "solid"; }
		if (item.locked) {var xtra='onmousedown="if (event.preventDefault) event.preventDefault()"';} else {var xtra=""}; // prevent default drag
		var cReturn = '<div id="' + _id + '" class="box pos type-verticalline" style="width: '+item.weight+'px;"><div class="inner liveUpdate-color" style="border-left: '+item.weight+'px '+item.borderStyle+' '+getColor(item.color)+';"></div></div>';
		return cReturn;
	}
	,propertyGroups: [
		{
...
prx.types.placeholder = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="bg liveUpdate-backgroundColor liveUpdate-borderColor" style="background-color: '+getColor(item.backgroundColor)+'; border-color: '+getColor(item.borderColor)+'; border-width: '+item.thickness+'px;"></div>';
		cR += '<div class="diagonal diagonal1 liveUpdate-borderColor" style="border-color: '+getColor(item.borderColor)+'; width: '+_width+'px; -moz-transform: rotate('+_angle1+'deg); -webkit-transform: rotate('+_angle1+'deg); -o-transform: rotate('+_angle1+'deg); transform: rotate('+_angle1+'deg);  border-top-width: '+item.thickness+'px; left: '+item.thickness/2+'px;"></div>'
		cR += '<div class="diagonal diagonal2 liveUpdate-borderColor" style="border-color: '+getColor(item.borderColor)+'; width: '+_width+'px; -moz-transform: rotate(-'+_angle1+'deg); -webkit-transform: rotate(-'+_angle1+'deg); -o-transform: rotate(-'+_angle1+'deg); transform: rotate(-'+_angle1+'deg); border-top-width: '+item.thickness+'px; right: '+item.thickness/2+'px;"></div>'
		//if(item.text != "") {
			cR += '<div class="contents"><span class="liveUpdate-backgroundColor liveUpdate-textColor" style="text-align: '+item.textAlign+'; font-size: '+item.textSize+'px; background-color: '+getColor(item.backgroundColor)+'; color: '+getColor(item.textColor)+';"><span data-editableproperty="text">' + item.text + '</span></span></div>';
		//}
		cR += '</div>';
		return cR;
	}
	...
	,propertyGroups: [
	    {
	    	...
	    	properties: [
		    	...
		    	[
		    		{ caption: 'Text', name: 'textSize', type: 'combo-select', value: function(item,name) { return item.textSize; }, values: prx.comps.textsize }
		    		//,prx.commonproperties.textAlign
		    		,{ 
		    			caption: false, 
		    			name: 'textColor', 
...
prx.types.tooltip = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#'+_id+' .tooltip-content { border-radius: '+item.borderRadius+'px; border: '+item.borderWidth+'px solid '+getColor(item.borderColor)+'; background-color: '+getColor(item.backgroundColor)+'; }'
		cR += '#'+_id+' .tooltip-text { '+_props+' '+getFontCssFromFontFamily(item.textFont)+'; font-size: '+item.textSize+'px; text-align: '+item.textAlign+'; color: '+getColor(item.textColor)+'; }'
		cR += '#'+_id+' .tooltip-outer { '+item.ttDirection+': 0; '+_pos+' }'
		cR += '#'+_id+' .tooltip { background: '+getColor(_ttBg)+'; '+item.ttDirection+': 0; border: '+item.borderWidth+'px solid '+getColor(item.borderColor)+'; margin: '+_margin+'; }'
		cR += '</style>'
		
		cR += '<div class="tooltip-content-outer">';
		cR += '<div class="tooltip-content liveUpdate-borderColor liveUpdate-backgroundColor">';
		cR += '<div class="tooltip-text liveUpdate-textColor"> <span data-editableproperty="text">'+item.text+'</span></div>'
		cR += '</div>';
		cR += '</div>';
		cR += '<div class="tooltip-outer">';
		cR += '<div class="tooltip liveUpdate-borderColor liveUpdate-backgroundColor"></div>'
		cR += '</div>'
		cR += '</div>';
	...
	,propertyGroups: [
		{
			...
			properties: [
				...
				,[
					{ caption: 'Border', name: 'borderWidth', type: 'combo-select', value: function(item,name) { return item.borderWidth; }, values: { min: 0, max: 10, step: 1 } }
			     	,prx.commonproperties.borderColor
...
prx.types.basic_tabbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#'+_id+' ul { background-color: '+getColor(item.backgroundColor)+'; }'
		cR += '#'+_id+' li { width: '+_tabWidth+'px; border-left: '+item.borderWidth+'px solid '+getColor(item.borderColor)+'; }'
		cR += '#'+_id+' label .caption { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' '+_props+'; font-size: '+item.textSize+'px; }'
		cR += '#'+_id+' input:checked + label { background-color: '+getColor(item.activeBackgroundColor)+'; }'
		cR += '#'+_id+' input:checked + label .caption { color: '+getColor(item.activeTextColor)+'; }'
		if(item.maskEnabled && $.browser.webkit) {
			cR += '#'+_id+' .icon { background-color: '+getColor(item.maskInactive)+'; -webkit-mask-size: auto '+(item.iconSize * 20)+'%; }'
			cR += '#'+_id+' input:checked + label .icon { background-color: '+getColor(item.maskActive)+' }'
		}
		switch (item.iconpos) {
			case 'top': 
		...
		cR += '<ul class="liveUpdate-backgroundColor">';
		$.each(item.tabs, function(i,elm){
			cR += '<li id="'+_id+'-tabs-'+i+'" class="dynamic-property liveUpdate-borderColor" data-dynamic-property-index="'+i+'">'
...
prx.types.generic_onoffswitch = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '#'+_id+' .onoffswitch-inner .active { background-color: '+getColor(item.activeLabelColor)+'; color: '+getColor(item.activeLabelTextColor)+'; }'
		cR += '#'+_id+' .onoffswitch-inner .inactive { background-color: '+getColor(item.inactiveLabelColor)+'; color: '+getColor(item.inactiveLabelTextColor)+'; }'
		cR += '#'+_id+' .onoffswitch-switch { background-color: '+getColor(item.switchColor)+'; width: '+item.switchSize+'px; border-radius: '+item.borderRadius+'px; margin: '+((item.height - item.switchSize)/2)+'px; right: '+(item.width-(Number(item.switchSize) + 4 + (item.height - Number(item.switchSize))))+'px; }'
		cR += '</style>'
