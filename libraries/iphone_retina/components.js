...
prx.types.toolbar_retina = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cReturn = '<div id="'+_id+'" class="box pos type-toolbar-retina" '+((item.overlay)? 'data-mpoverlay="1"': '')+'><div class="inner liveUpdate-backgroundColor" style="background-color: '+getColor(item.backgroundColor)+';"></div></div>';
		return cReturn;
	}
	,propertyGroups: [
	                  {
...
prx.types.tabbar_retina = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.tabs, function(i,elm){
			...
			cR += '<label for="'+_id+'-radio-'+i+'"><em style="'+_icon+'+"><img src="'+getAssetUrl(elm.icon)+'" /></em><span data-editableproperty="caption" data-dynamic-property-index="'+i+'">'+elm.caption+'</span></label>';
			cR += '</li>'; 
		});
		cR = cR +  '</ul>';
		cR = cR + '</div>';
...
prx.types.button2_retina = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		*/
		...
		if(item.arrowDirection != 'none') {
			cR = cR + '<div class="button-arrow " style="overflow: hidden; height: 100%; width: '+Math.round(_dims.height/2)+'px; '+item.arrowDirection+': 0;"><div  class="liveUpdate-borderColor liveUpdate-backgroundColor" style="height: '+arrow_h+'px; width: '+arrow_h+'px; '+arr+' top: '+arrow_pos+'px; '+bgArrCss+'"></div></div>';
		}
		...
		case 'right':
			if(item.buttonicon.url != '') {
				cR = cR + '<img src="'+getAssetUrl(item.buttonicon)+'" style="position: absolute; '+item.iconpos+': 16px; height: '+eval((_dims.height-item.borderWidth*2)*0.2*item.iconSize)+'px; padding-top: '+eval((_dims.height-item.borderWidth*2)*0.1*(5-item.iconSize))+'px; " />';
			}
			break;
		case 'notext':
...
prx.types.radiobutton_retina = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<label class="liveUpdate-backgroundColor" for="'+_id+'-checkbox" style="'+_bg+'"><span class="liveUpdate-backgroundColor" style="'+_bg+'"></span></label>';
		cR += '</div>';
		return cR;
	}
	,interactions: [
...
prx.types.checkboxlist_retina = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="label-container liveUpdate-backgroundColor liveUpdate-textColor liveUpdate-borderColor '+_hor+'" style="text-align: '+item.textAlign+';background-color: '+getColor(item.backgroundColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; color: '+getColor(item.textColor)+'; border-color: '+getColor(item.borderColor)+'; '+cssBorderRadius(item.borderRadius + 'px')+_props+'">';
		$.each(item.checkboxes, function(i,elm){
			...
			cR += '<label data-dynamic-property-index="'+i+'" for="'+_id+'-checkbox-'+i+'" class="dynamic-property liveUpdate-borderColor '+((elm.active) ? 'liveUpdate-activeBackgroundColor liveUpdate-activeTextColor' : '')+'" style="'+_style+' border-color: '+getColor(item.borderColor)+';"><span data-editableproperty="text" data-dynamic-property-index="'+i+'">' +elm.text+ '</span>' + _checkmark + '</label>'
			cR += '</span>';
		});
		cR += '</div></div>';		
		return cR;
...
prx.types.listcomplex_retina = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		$.each(item.listitems, function(i,elm){		
			...
			cR += '<li '+_style+' id="'+_id+'-listitems-'+i+'" class="dynamic-property liveUpdate-borderColor listitem-type-'+elm.itemtype+'" data-dynamic-property-index="'+i+'">';
...
prx.types.actionsheet_retina = {
	...
	,onDisplay: function(item,containerid) {
		...
		var cR = '';
		cR += '<div id="' + _id + '" class="box pos type-actionsheet-retina"><div class="actionsheet-inner liveUpdate-backgroundColor" style="padding: '+item.padding+'px; '+_bg+' background-size: 100% 40px; background-repeat: no-repeat; background-color: '+getColor(item.backgroundColor)+';">';
		$.each(item.buttons, function(i,elm){
...
prx.types.popover_retina = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="popover-content liveUpdate-backgroundColor liveUpdate-borderColor-border-color" style="height: '+_height2+'; '+_borderRadius+' border: 10px solid '+getColor(item.borderColor)+'; background-color: '+getColor(item.backgroundColor)+'"></div>';
		cR += '</div></div>';
